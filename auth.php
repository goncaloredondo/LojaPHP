...
require 'db.php'; // Inclusão do arquivo de conexão com o banco de dados
require 'email.php'; // Inclusão do arquivo de manipulação de e-mails
...
{
    session_start();
    require 'db.php'; // Certifica que este ficheiro conecta à base de dados
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        $username = $_POST["username"];
        $password = $_POST["password"];
        ...
        if ($result->num_rows > 0)
        {
            ...
            if (password_verify($password, $row["password"]))
            {
                ...
                header("Location: ../views/loja.php");
                exit;
            }
            else
            {
...
function recuperacao_senha($email)
{
    ...
    if($result->num_rows > 0)
    {
        ...
        if ($sql_update->affected_rows > 0)
        {
            ...
            require_once '../views/reset_senha.php';
...
function registo($email,$username,$password,$telemovel,$nif)
{
        ...
        if ($sql->execute())
        {
            ...
            sleep(2);
            require_once '../views/validar_registo.php';
        }
        else
        {
